  ...
  useEffect(() => {
    const extractContactInfo = () => {
      try {
        ...
        const contactInfoHeader = Array.from(document.querySelectorAll('div')).find(el => 
          el.textContent.trim() === 'Contact info'
        );
        
        if (!contactInfoHeader) return;

        const contactInfoPanel = contactInfoHeader.closest('header')?.parentElement?.parentElement;
  ...
  const handleSendMessage = async (message) => {
    ...
    if (!messageSent) {
      try {
        const personalizedMessage = message.replace('[Customer]', contactName);
        await navigator.clipboard.writeText(personalizedMessage);
        alert('Message copied to clipboard! Please paste (Ctrl+V) it in WhatsApp');
      } catch {
        alert(`Failed to send. Here's the message:\n\n${message.replace('[Customer]', contactName)}`);
      }
    }
  };
            ...
            {category.messages.map((msg, i) => (
              <button
                ...
                style={{
                  ...
                  ':hover': {
                    background: isLoadingContact ? 'transparent' : theme.accent + '15',
                    borderColor: isLoadingContact ? theme.border : theme.accent,
                  },
                }}
              >
                {isLoadingContact ? 'Loading...' : msg.replace('[Customer]', contactName)}
              </button>
            ))}
          </div>
        </div>
